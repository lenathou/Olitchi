    ...
    <section className={`py-16 lg:py-24 ${className}`}>
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <motion.h2
            className="text-3xl md:text-4xl lg:text-6xl font-bold mb-6 leading-tight font-serif"
            variants={fadeUp}
          >
        ...
        {/* ── Options Grid (scroll reveal + stagger) ── */}
        <motion.div
          className={`grid gap-8 mb-16 ${isMobile ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-3'}`}
          ...
          {ctaOptions.map((option, index) => (
            <motion.div key={index} variants={cardReveal}>
              <motion.div whileHover={shouldReduceMotion ? undefined : cardHover}>
                <Card
                  variant="bubble"
                  className={`group transition-all duration-300 relative overflow-hidden rounded-[2rem] border-2 ${option.highlight ? 'border-primary/20 bg-primary/5' : ''
                    }`}
                >
                  {option.highlight && (
        ...
        {/* ── Testimonial (scroll reveal + scale) ── */}
        <motion.div variants={scaleReveal} {...revealProps}>
